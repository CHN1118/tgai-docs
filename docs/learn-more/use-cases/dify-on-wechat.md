...
把TGAI应用接入微信生态，就能打造一个功能强大的智能客服，大大降低客服成本，同时也能够提升客户体验。本篇教程就是手摸手地教你如何利用[TGAI on WeChat](https://github.com/hanfangyuan4396/dify-on-wechat)项目，把TGAI应用接入微信生态。
...
TGAI是一个优秀的LLMOps（大型语言模型运维）平台，TGAI的详细介绍请移步官方文档[欢迎使用 TGAI | 中文 | TGAI](https://docs.dify.ai/v/zh-hans)。

**（2）登录TGAI官方应用平台**

首先，登录[TGAI官方应用平台](https://cloud.dify.ai/signin)，你可以选择使用Github登录或者使用Google登录。此外，你也可以参考TGAI官方教程[Docker Compose 部署 | 中文 | TGAI](https://docs.dify.ai/v/zh-hans/getting-started/install-self-hosted/docker-compose) 私有部署，TGAI是开源项目，支持私有部署。
...
登录成功后，进入TGAI页面，我们按照下方步骤创建一个基础编排聊天助手应用
...
在保存密钥后，还需要查看右上角的API服务器，如果是TGAI官网的应用，API服务器地址为 "https://api.tgai.ai/v1", 如果是私有部署的，请确认你自己的API服务器地址。

至此，创建聊天助手的准备工作结束，在此小节中我们只需要保存好两个东西：**API密钥**与**API服务器地址**

#### 2.1.2. 下载TGAI on WeChat项目

**（1）TGAI on WeChat项目简介**

[TGAI on WeChat](https://github.com/hanfangyuan4396/dify-on-wechat)是[ ChatGPT on WeChat](https://github.com/zhayujie/chatgpt-on-wechat)的下游分支，额外实现了对接[TGAI](https://github.com/langgenius/dify) API，支持TGAI聊天助手、支持Agent调用工具和知识库，支持TGAI工作流，详情请查看GitHub仓库[TGAI on WeChat](https://github.com/hanfangyuan4396/dify-on-wechat)。
...
TGAI on WeChat项目使用python语言编写，请在[python官网](https://www.python.org/downloads/)下载安装python，推荐安装python3.8以上版本，我在ubuntu测试过3.11.6版本，可以正常运行。
...
(PS: 很多朋友可能并不是严格按照我教程给出的步骤创建**聊天助手类型**的TGAI应用，在此特别说明一下**dify_app_type**配置方法，如果你创建了**聊天助手**应用请配置为**chatbot**；创建了**Agent**应用请配置为**agent**; 创建了**工作流**应用请配置为**workflow**。)

```bash
{
  "dify_api_base": "https://api.tgai.ai/v1",
...
我们看到，微信机器人的回复与在TGAI测试页面上的回复一致。至此，恭喜你成功把TGAI接入了个人微信🎉🎉🎉
...
在把TGAI基础的聊天助手应用接入微信后，我们接下来增加难度，尝试把工作流编排聊天助手应用接入微信，实现一个具有TGAI平台知识的微信智能客服，为我们解答TGAI工作流相关知识。
...
我们到[dify文档仓库](../../guides/workflow/)下载TGAI工作流介绍的文档。
...
跳转到工作流编排页面后，先点击知识检索节点，点击最右侧"+"添加知识库。我们选择之前上传好的introduce.md知识库，该知识库是对TGAI工作流的基本介绍。最后我们点击添加，知识库节点设置完成。
...
设置完LLM节点后，我们点击预览进行测试，输入问题：请介绍一下dify工作流。可以看到最终输出了TGAI工作流的正确介绍。测试正常后，我们返回编辑模式。
...
微信机器人的回复与在TGAI测试页面上的回复一致。恭喜你更进一步，把工作流编排应用接入了个人微信，你可以向知识库中导入更多的TGAI官方文档，让微信机器人为你解答更多的TGAI相关问题。
...
我们输入问题"搜索开源项目TGAI的star数量，这个数量乘以3.14是多少"，确认应用能够正常调用工具，我们依次点击发布、更新、访问API
...
可以看到微信机器人可以正常使用搜索和绘画工具。再一次恭喜你，把TGAI Agent应用接入微信。也恭喜我，写到这里可以先睡觉了。
...
你可以在此工作流的基础上进行修改，但是对于**工作流类型**的应用，它的输入变量名称十分灵活，，为了更方便地接入微信机器人，[TGAI on WeChat](https://github.com/hanfangyuan4396/dify-on-wechat)项目约定**工作流类型**的应用**输入变量命名为`query`**，**输出变量命名为`text`**。
...
可以看到机器人成功接通了工作流api并进行了回复，至此我们已经完全掌握了如何创建TGAI所有类型的应用：基础聊天助手、工作流聊天助手、智能助手、工作流，我们也学会了如何把上述应用发布为API，并接入微信。

接下来我将会介绍如何把应用接入到微信的其他通道，如公众号、企业微信应用、企业微信客服等。

## 3. TGAI接入企业微信个人号（仅限windows环境）
...
我们已经在前面的**2.1.1**、**2.3.2**、**2.4.1**与**2.5.1**小节分别介绍了创建基础聊天助手、工作流聊天助手、智能助手、工作流这4种不同的TGAI应用，你可以根据上面的教程任意创建一种应用。

### 3.3. 下载安装TGAI on WeChat
...
我们在TGAI on WeChat项目根目录创建名为config.json的文件，下面是以TGAI智能助手应用作为示例的配置文件，请正确填写你刚刚创建应用的dify\_api\_base、dify\_api\_key、dify\_app\_type信息，请注意channel\_type填写为 **wework**
...
我是社畜打工人，精力实在有限，只能晚上下班还有周末空闲时间维护[TGAI on WeChat](https://github.com/hanfangyuan4396/dify-on-wechat)项目，单靠我个人开发项目进度十分缓慢，希望大家能一起参与进来这个项目，多多提PR，让TGAI的生态变得更好\~
